...
function Cifra({ testId, children }: { testId: string; children: ReactNode }) {
  return (
    <p data-testid={testId} className="text-2xl font-semibold tabular-nums text-slate-900">
      {children}
    </p>
  );
}
...
const COLOR_FRESCURA: Record<Frescura, string> = {
  ...
  demorada: 'text-amber-700',
  desconectada: 'text-red-700',
};
    ...
    <div className="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <Tarjeta titulo="Mesas abiertas">
        ...
        <p className="text-sm text-slate-500">
          En curso:{' '}
          <span data-testid="kpi-en-curso" className="font-medium text-slate-700">
        ...
        {sinContar && <p className="mt-1 text-xs text-amber-700">{sinContar}</p>}
      </Tarjeta>
      ...
      <Tarjeta titulo="Atención >60 min">
        ...
        {kpis.sinHora > 0 && (
          <p data-testid="kpi-sin-hora" className="text-xs text-amber-700">
            {kpis.sinHora === 1
...
function TarjetaMesa({
  ...
  return (
    ...
    >
      ...
      {conSucursal && <p className="truncate text-xs text-slate-500">{mesa.sucursal}</p>}
      <p className="truncate text-sm text-slate-600">{mesa.mesero ?? 'Mesero: sin dato'}</p>
      ...
      {mesa.partidas === null ? (
        <p className="mt-2 text-xs text-slate-500">Partidas: sin dato</p>
      ) : (
        <ul className="mt-2 space-y-0.5 text-xs text-slate-700">
          ...
          {partidas.length === 0 && <li className="text-slate-500">Sin partidas</li>}
        </ul>
      )}
    </li>
  );
